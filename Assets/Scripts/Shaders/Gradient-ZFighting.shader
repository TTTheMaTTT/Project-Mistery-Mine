Shader "MysteryMine/Gradient/ZFighting" 
{
	...
	Properties {
		...
		[Enum(UnityEngine.Rendering.BlendMode)] _BlendDst("Blend Destination", Float) = 1 // Параметры смешивания
	...
	SubShader {
		...
		Pass { 
			...
			Blend[_BlendSrc][_BlendDst]			//SrcFactor DstFactor (also:, SrcFactorA DstFactorA) = One | Zero | SrcColor | SrcAlpha | DstColor | DstAlpha | OneMinusSrcColor | OneMinusSrcAlpha | OneMinusDstColor | OneMinusDstAlpha - Blending between shader output and the backbuffer will use blend mode 'Alpha Blend'
